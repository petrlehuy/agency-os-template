...
## Hand-Drawn Style Pipeline (Flow Diagrams)

All flow diagrams (strategy flows, email sequences, automations) use the **hand-drawn style** as the default output format. This matches the reference template in `reference/flow-strategy-template/`.

### Visual Properties
- **Font:** Virgil (fontFamily: 1) — hand-drawn look
- **Roughness:** 1 — single sketchy line (middle sloppiness)
- **Stroke width:** 4 — bold hand-drawn strokes
- **Font size:** 20px universal, 26px for endnodes
- **Decision ellipses:** 168×97 (wider horizontal ovals)
- **Endnode ellipses:** 152×91
- **Arrow bindings:** `focus: 0, gap: 5` — renderer auto-centers connections on shape edges
- **Arrow curves:** Natural midpoints proportioned from reference (37-58% of vector)
- **ANO/NE labels:** Offset 20px right of arrow start, not overlapping arrows

### Pipeline (run in order)

```bash
# 1. Reset to clean base state (Helvetica, roughness 0)
python3 tools/fix-quality.py

# 2. Apply hand-drawn style transformation
python3 tools/handdrawn-style.py <path-to-file.excalidraw>
# Or for all flows:
for f in projects/<client>/outputs/flow-diagrams/*.excalidraw; do
  python3 tools/handdrawn-style.py "$f"
done
...
- **fix-quality.py** resets fonts/roughness/strokeWidth but does NOT reset element sizes or bindings — safe to re-run
- **handdrawn-style.py** uses absolute target dimensions (idempotent — no compounding on re-run)
- Arrow bindings use `focus/gap` format (NOT `fixedPoint/orbit`) because `exportToSvg` renderer natively supports focus/gap and auto-recalculates connection points
- The reference template is at `reference/flow-strategy-template/flow example.excalidraw` (with .png and .svg for visual comparison)

### When NOT to Use

- Conceptual/technical diagrams that aren't flow charts — use the standard clean style (roughness: 0, fontFamily: 3)
- One-off quick diagrams where hand-drawn feel isn't needed
